...
export default function AccountsPanel({ loadState, connections, accounts, executions, schedules, workerHealth, dailySummary, autonomousControls }: Props) {
  ...
  if (!connections.length) return <section className="empty-state workspace-empty"><div><div className="eyebrow">Broker workspace</div><h2>Connect TradeLocker to begin</h2><p>No stored connection was found. Add one to discover accounts and create execution profiles.</p></div><a className="button" href="/connect-tradelocker?new=1">Add connection</a></section>;
...
function AccountCard({ account, schedules, busy, profileStatus, checkStatus, mutate, openDialog }: { account: AccountSummary; schedules: ScheduleSummary[]; busy: boolean; profileStatus: Record<string, string>; checkStatus: (profile: ProfileSummary) => Promise<void>; mutate: (path: string, method?: "POST" | "PUT" | "PATCH" | "DELETE", body?: object) => Promise<boolean>; openDialog: (dialog: Dialog) => void }) {
  return <article className={`account-card ${account.is_default_analysis ? "is-selected" : ""}`}>
    <header className="account-header"><div><div className="account-title-row"><h4>{account.account_alias}</h4>{account.is_default_analysis && <span className="selected-tag">Selected</span>}</div><p>{account.account_name || `${displayBroker(account.broker_name)} account`} · {account.currency || "Currency unknown"}</p></div><div className="badges"><StatusBadge value={account.is_demo === 1 ? "demo" : account.is_demo === 0 ? "live" : "unknown"} /><StatusBadge value={account.available && account.locally_enabled ? "active" : "unavailable"} /></div></header>
